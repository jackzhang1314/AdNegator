...
E2B 官方沙盒生命周期采用**简化 4 态模型**（无 pause/resume）：

| 状态 | 英文 | 描述 | 持久化 | Firecracker 状态 |
|------|------|------|--------|------------------|
| 创建中 | `creating` | Firecracker microVM 启动中，envd 初始化 | ✅ | `NotStarted` → `Running` |
| 运行中 | `running` | microVM 正常运行，envd gRPC 可访问 | ✅ | `Running` |
| 销毁中 | `terminating` | 正在删除 Firecracker VM 和 Nomad Job | ✅ | `Stopping` |
| 已销毁 | (deleted) | 数据库记录已删除，不可查询 | ❌ | `NotStarted` |
...
stateDiagram-v2
    ...
    creating --> running: envd health check OK
    creating --> terminating: timeout (800ms) / fatal error

    running --> terminating: DELETE /sandboxes
    running --> terminating: timeout (default 1h)
    running --> terminating: OOM / panic
